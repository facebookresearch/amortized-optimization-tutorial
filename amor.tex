...
The term \emph{amortization} originates in the
variational inference community \citep{kingma2013auto,rezende2014stochastic,stuhlmuller2013learning,gershman2014amortized,webb2017faithful,ravi2018amortized,cremer2018inference,wu2020meta},
and has been expanded to more general optimization settings
\citep{xue2020amortized,sercu2021neural,xiao2021amortized}.
While there are other general formulations of
learning to optimize, such as
\citet{li2016learning,chen2021learning,heaton2021learning},
we will connect to these later and instead follow
\citet{shu2017amortized} for defining the core foundation
of amortized optimization as:
...
% We now turn to foundations for crafting
% the model and loss, and then move on to concretely
% instantiate them in \cref{sec:apps}.
...
that can leverage domain knowledge.
...
\citet{gregor2010learning} integrates ISTA iterates
...
iterate $\hat y^0_\theta(x)=\theta$ and takes one gradient step:
\begin{equation}
...
\end{equation}
where the base $\hat y_\theta^0(x)=\theta$ as before.
...
such as Bregman projections, are \emph{not} idempotent and
often $\pi_\gC\circ \pi_\gC\neq \pi_\gC$
...
They propose two new human-interpretable optimization methods
their models uncovered.
...
\citep{kingma2013auto,rezende2014stochastic,mnih2014neural,rezende2015variational,higgins2016beta,doersch2016tutorial}
...
These models model the density $p(x)$ over
a high-dimensional space, for example images, text, or
videos, given samples $x\sim p(x)$.
...
\citep{byravan2019imagined,hafner2019dream,byravan2020imagined,byravan2021evaluating,amos2021model}.
As observed in
...
\item \citet{weng2018metalearning} is a long blog post
...
seismic imaging.
\item \citet{hospedales2020meta} is a reviewed focused on meta-learning,
...
\subsection{Learning-augmented algorithms beyond optimization}
