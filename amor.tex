...
\item The \emph{ReLU}, defined by $f(x) \defeq \max\{0, x\}$,
  ...
  orthant $\R^n_+$
  and is the solution to the optimization problem
  \begin{equation}
...
\item The \emph{sigmoid} or \emph{logistic} function, defined by $f(x) \defeq (1+e^{-x})^{-1}$,
  ...
  the interior of the unit hypercube $(0,1)^n$
  and is the solution to the optimization problem
  \begin{equation}
...
\item The \emph{softargmax}, historically referred to as the
  \emph{softmax}, defined by $f(x)_j = e^{x_j} / \sum_i e^{x_i}$,
  projects a point $x\in\R^n$ onto
