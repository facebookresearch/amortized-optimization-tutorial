...
\noindent Other projections in machine learning include:
\begin{itemize}
...
\emph{fixed-point residual norm} $\gR(y)\defeq\|g(y)-y\|_2^2$ with
\begin{equation}
  y^\star(x)\in\argmin_y \gR(y),
...
$\gR(y; x)\defeq ||y-g_\varphi(y;x)||_2^2$
as
\begin{equation}
  y^\star(x)\in \argmin_x \gR(y; x).
  \label{eq:deq-opt}
\end{equation}
