...
% TODO: Other graphical visualization of amortization?
...
\begin{tikzpicture}[every path/.style={thick}]
  ...
  \node[obs] (x) {$x$};
  \node[obs,right=.6cm of x,fill=red!20] (y) {$y^\star$};
  \node[latent,above=0.6cm of y] (theta) {$\theta$};
  \scoped[on background layer]{
    \plate [inner xsep=.6cm,fill=gray!15,xshift=.5cm] {plate} {(x) (y)} {};
  };
  \edge {x} {y};
  \edge {theta} {y};
  \node[right=-10mm of plate.east] {$p(x)$};
\end{tikzpicture}
\vspace{-4mm}
\end{center}
\captionsetup{font={small}}
\caption{Graphical view of amortized optimization.}
\label{fig:amor-graph}
\vspace{-4mm}
\end{wrapfigure}
\textbf{Graphical amortized optimization models.}
I propose to visualize the amortized optimization settings
with a diagram such as \cref{fig:amor-graph}, which shows how
the optimal solution $y^\star(x)$ depends on $x$ and captures
the latent structure across $p(x)$, the distribution of contexts,
with the parameters $\theta$.
These diagrams are especially useful for visualizing
the dependencies and shared structure in specialized
amortization settings with more components.
...
% TODO: Better diagram or more intuition on why we would want a latent
% space, e.g. to search only over the optimal solutions.
\begin{center}
\begin{tikzpicture}
  \matrix (m) [
      matrix of math nodes,row sep=2em,column sep=1em,
...
\caption{Optimizing \cref{eq:maxent}.
  ...
  \href{https://github.com/facebookresearch/amortized-optimization-tutorial/blob/main/fig/maxent.gif}{here}.
}
% \vspace{-2mm}
\end{wrapfigure}
...
\label{sec:apps:avi}
% TODO: Figure of the model/components of the VAE?
...
\subsection{Amortized $c$-transforms, conjugation,
  and coupling in optimal transport}
\label{sec:apps:ot}
\textbf{Preliminaries.}
...
solves
\begin{equation}
  W(\alpha, \beta)\defeq \min_{\pi\in\gU(\alpha, \beta)} \int_{\gX\times \gY} c(x,y){\rm d}\pi(x,y),
  \label{eq:kantorovich}
\end{equation}
where the \emph{coupling} $\pi$ is a joint distribution over the product of the measures,
...
can be represented by
\begin{equation}
  \max_{f} g(f) \quad \textrm{with dual objective}\quad g(f)\int_\gX f(x){\rm d}\alpha(x) + \int_\gY f^*(y) {\rm d}\beta(y)
...
In Euclidean spaces, $f$ is a convex function and the $c$-transform
...
a linear programming formulation or sinkhorn iteratinos
...
Meta OT speeds up optimal transport solvers by orders of magnitude by
amortizing the Kantorovich dual computation in \cref{eq:kantorovich-dual}.
...
\citet{makatura2021paretogamuts} investigate amortizing Pareto
frontier computations for engineering design problems.
